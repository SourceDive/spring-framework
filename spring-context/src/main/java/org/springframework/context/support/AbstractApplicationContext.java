...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 上面这个注释的断句: Invoke factory processors (registered as beans 这个是修饰前面的 factory processors 的)
				// in the context.
				// 01、调用工厂处理器
				...
				// 初始化及注册 bean 后置处理器
				// 一个需要注意的就是切面需要的 post processor 就是在这里注册的
				registerBeanPostProcessors(beanFactory);
				beanPostProcess.end();

				// Initialize message source for this context.
				// @ongoing 这个 message source 在书中叫作国际化组件
				initMessageSource();
	...
	// 初始化完 bean factory 后，才开始初始化单例 bean
	/**
	 ...
	 */
	@SuppressWarnings("deprecation") // 抑制弃用方法警告
	 ...
	 * @throws IllegalStateException if already initialized and multiple refresh ({@link GenericApplicationContext} 不支持多次刷新)
